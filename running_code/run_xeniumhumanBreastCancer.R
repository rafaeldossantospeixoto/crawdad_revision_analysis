...
ggplot(df, aes(x = x, y = y, col = celltype)) +
  coord_fixed() +
  rasterise(geom_point(size = 0.3, stroke = 0.01), dpi = 300) +
...
vizColocDotplot(dat_filtered, zsig.thresh = zsig, zscore.limit = 2*zsig, reorder = TRUE, dot.sizes = c(3, 15)) +
  coord_fixed() +
  theme(
...
for (celltypes in niches) {
  ...
  ggplot(df, aes(x = x, y = y)) +
    coord_fixed() +
    rasterise(geom_point(color = "lightgray", size = 0.3, stroke = 0), dpi = 300) +
    rasterise(geom_point(data = df_sub, aes(x = x, y = y, col = celltype), size = 0.5, stroke = 0), dpi = 300) +
    scale_color_manual(values = col_ct[celltypes], name = "cell types") +
    guides(col = guide_legend(override.aes = list(size = 3))) +
